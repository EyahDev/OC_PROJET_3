...
class ControleurProfiladmin extends ControleurSecurise {
    ...
    public function modifierPassword() {
        ...
        if ($this->utilisateur->verifUtilisateur($login, $ancienPassword)) {
            ...
            // Vérification si le nouveau mot de passe et la vérification du nouveau mot de passe sont strictement égals
            ...
            // Redirection vers la page de modification du mot de passe pour un nouvel essai
            $this->redirection('profiladmin/password');
        }
    }
    ...
    public function modifierMail () {
        ...
        if (empty($nvMail)) {
            ...
            // Vérification si une modification a eu lieu ou non et génère un message flash en conséquence
            if ($maj == 1) {
                $this->requete->getSession()->setMessageFlash('confirmation', 'Votre adresse mail a bien été modifiée');
            } else {
                $this->requete->getSession()->setMessageFlash('confirmation', 'Aucune modification n\'a été appliquée');
            }
    ...
    public function publierApropos () {
        ...
        if (empty($aPropos)) {
            ...
            // Vérification si une modification a eu lieu ou non et génère un message flash en conséquence
            if ($maj == 1) {
                $this->requete->getSession()->setMessageFlash('confirmation', 'La section "A propos" a bien été publiée');
            } else {
                $this->requete->getSession()->setMessageFlash('confirmation', 'Aucune modification n\'a été appliquée');
            }
